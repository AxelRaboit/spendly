/**
 * Marked extension for Obsidian-style callouts.
 *
 * Syntax:
 *   > [!type] Optional title
 *   > Body content here
 *
 * Supported types: note, tip, info, warning, danger, bug, example, quote, success, question, abstract
 */
const CALLOUT_TYPES = {
    note: { icon: 'pencil', label: 'Note' },
    tip: { icon: 'flame', label: 'Tip' },
    hint: { icon: 'flame', label: 'Hint' },
    info: { icon: 'info', label: 'Info' },
    warning: { icon: 'alert-triangle', label: 'Warning' },
    caution: { icon: 'alert-triangle', label: 'Caution' },
    danger: { icon: 'zap', label: 'Danger' },
    bug: { icon: 'bug', label: 'Bug' },
    example: { icon: 'list', label: 'Example' },
    quote: { icon: 'quote', label: 'Quote' },
    cite: { icon: 'quote', label: 'Cite' },
    success: { icon: 'check', label: 'Success' },
    question: { icon: 'help-circle', label: 'Question' },
    faq: { icon: 'help-circle', label: 'FAQ' },
    abstract: { icon: 'clipboard-list', label: 'Abstract' },
    summary: { icon: 'clipboard-list', label: 'Summary' },
    todo: { icon: 'check-circle', label: 'Todo' },
    failure: { icon: 'x', label: 'Failure' },
    fail: { icon: 'x', label: 'Fail' },
    missing: { icon: 'x', label: 'Missing' },
};

const SVG_ICONS = {
    pencil: '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg>',
    flame: '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z"/></svg>',
    info: '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>',
    'alert-triangle':
        '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg>',
    zap: '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>',
    bug: '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m8 2 1.88 1.88"/><path d="M14.12 3.88 16 2"/><path d="M9 7.13v-1a3.003 3.003 0 1 1 6 0v1"/><path d="M12 20c-3.3 0-6-2.7-6-6v-3a4 4 0 0 1 4-4h4a4 4 0 0 1 4 4v3c0 3.3-2.7 6-6 6"/><path d="M12 20v-9"/><path d="M6.53 9C4.6 8.8 3 7.1 3 5"/><path d="M6 13H2"/><path d="M3 21c0-2.1 1.7-3.9 3.8-4"/><path d="M20.97 5c0 2.1-1.6 3.8-3.5 4"/><path d="M22 13h-4"/><path d="M17.2 17c2.1.1 3.8 1.9 3.8 4"/></svg>',
    list: '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="8" x2="21" y1="6" y2="6"/><line x1="8" x2="21" y1="12" y2="12"/><line x1="8" x2="21" y1="18" y2="18"/><line x1="3" x2="3.01" y1="6" y2="6"/><line x1="3" x2="3.01" y1="12" y2="12"/><line x1="3" x2="3.01" y1="18" y2="18"/></svg>',
    quote: '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 21c3 0 7-1 7-8V5c0-1.25-.756-2.017-2-2H4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2 1 0 1 0 1 1v1c0 1-1 2-2 2s-1 .008-1 1.031V21c0 1 0 1 1 1z"/><path d="M15 21c3 0 7-1 7-8V5c0-1.25-.757-2.017-2-2h-4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2h.75c0 2.25.25 4-2.75 4v3c0 1 0 1 1 1z"/></svg>',
    check: '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>',
    'help-circle':
        '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><path d="M12 17h.01"/></svg>',
    'clipboard-list':
        '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="8" height="4" x="8" y="2" rx="1" ry="1"/><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><path d="M12 11h4"/><path d="M12 16h4"/><path d="M8 11h.01"/><path d="M8 16h.01"/></svg>',
    'check-circle':
        '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>',
    x: '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>',
};

function getCalloutType(type) {
    return CALLOUT_TYPES[type.toLowerCase()] ?? { icon: 'info', label: type };
}

function getIcon(iconName) {
    return SVG_ICONS[iconName] ?? SVG_ICONS.info;
}

export function createCalloutExtension() {
    return {
        name: 'callout',
        level: 'block',
        start(src) {
            return src.match(/^>\s*\[!/)?.index;
        },
        tokenizer(src) {
            // Match > [!type] optional title\n(> lines...)
            const match = src.match(/^(?:>\s*\[!(\w+)\]\s*(.*)\n)((?:>.*(?:\n|$))*)/);
            if (!match) return;

            const type = match[1];
            const title = match[2].trim();
            // Strip leading > and optional space from body lines
            const bodyRaw = match[3]
                .split('\n')
                .map((line) => line.replace(/^>\s?/, ''))
                .join('\n')
                .trim();

            // Pre-lex body tokens here (this.lexer is only available in the tokenizer)
            const tokens = [];
            if (bodyRaw) {
                this.lexer.blockTokens(bodyRaw, tokens);
            }

            return {
                type: 'callout',
                raw: match[0],
                calloutType: type,
                title: title,
                tokens,
            };
        },
        renderer(token) {
            const config = getCalloutType(token.calloutType);
            const icon = getIcon(config.icon);
            const title = token.title || config.label;
            const escapedTitle = title.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
            const typeLower = token.calloutType.toLowerCase();

            // Parse body using pre-lexed tokens (this.lexer is not available in renderer)
            const bodyHtml = token.tokens?.length ? this.parser.parse(token.tokens) : '';

            return `<div class="callout callout-${typeLower}">
  <div class="callout-header">
    <span class="callout-icon">${icon}</span>
    <span class="callout-title">${escapedTitle}</span>
  </div>
  ${bodyHtml ? `<div class="callout-body">${bodyHtml}</div>` : ''}
</div>\n`;
        },
    };
}
